...
class HomeController extends Controller
{
    public function index(){
       ...
       if(Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%about%")->where('page_type','Group')->latest()->first()!=null){
        ...
        // }
        //return $misson;
        $job_categories = Navigation::query()->where('page_type','Job')->orderBy('updated_at', 'desc')->paginate(6);
        // if($category->count()>0){
        ...
        //return $missons;       
        return view("website.index")->with(['newsbanner'=>$newsbanner,'job_categories'=>$job_categories,'testimonial'=>$testimonial,'statistics'=>$statistics,'banners'=>$banners,'partners'=>$partners,'about'=>$About,'menus'=>$menus,'global_setting'=>$global_setting,'sliders'=>$sliders,'missons'=>$missons,'message'=>$message,'jobs'=>$jobs]);
    }
    ...
    public function category($menu){
        ...
        $job_categories = Navigation::query()->where('page_type','Group Jobs')->where('page_status',1)->orderBy('position', 'asc')->paginate(6);
        $global_setting = GlobalSetting::all()->first();        
        if(Navigation::all()->where('nav_name',$menu)->count()>0){

            $jobs = Navigation::query()->where('page_type','Job')->where('parent_page_id',$category_id)->where('page_status',1)->orderBy('created_at', 'desc')->paginate(6);
        }
        else{
            $jobs = null;
        }
        ...
        if($category_type == "Photo Gallery"){
            ...
            $photos = NavigationItems::where('navigation_id',$category_id)->paginate(8);
        ...
        elseif($category_type == "Normal"){
            $normal = Navigation::find($category_id);
            if ($normal->parent_page_id == 2546) {
                $parent_data = Navigation::query()->where('parent_page_id', $normal->parent_page_id)->get();
            }
            else {
                $parent_data = Null;
            }
            // return $parent_data;
            return view("website.normal")->with(['normal'=>$normal,'parent_data'=>$parent_data,'jobs'=>$jobs,'menus'=>$menus,'sliders'=>$sliders,'about'=>$About,'global_setting'=>$global_setting,'slug1'=>$slug1]);
        }
